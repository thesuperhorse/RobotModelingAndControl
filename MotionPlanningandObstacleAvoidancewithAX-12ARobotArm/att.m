...
%Takes curren configuration q, target configuration qf, and robot structure myrobot. Returns the normalized joint torques resulting from attractive forces.
    H_start = forward_ax12(myrobot, q);                                  
    H_end = forward_ax12(myrobot, qf);                                   
    ...
    origin_final = {H01f(1:3,4), H02f(1:3,4), H03f(1:3,4), H04f(1:3,4)};
       
    
    F_vec = [];
    for j = 1:4                                                 %loop through joints to calculate attractive forces
